...
/**
 * Default event to play with.
 */
class LoadEventEntriesData extends DataFixture
{
    /**
     * {@inheritdoc}
     */
    public function load(ObjectManager $manager)
    {
        ...
        for ($i = 1; $i <= 50; $i++) {
            $post = $this->get('free_note.repository.event_entry')->createNew();
            ...
            $categories = array(
                $this->getReference('Sandbox.Event.Category.'.$randomA),
                $this->getReference('Sandbox.Event.Category.'.$randomB)
            );
            ...
            $this->setReference('Sandbox.Event.Entry-'.$i, $post);
